    ...
    if(!cubesAreScared){
        if(!worldIs3D){ 
            ...
            for(float x = -6.0; x <= 6.0001; x+= 1.0){
                for (float y = -6.0; y <= 6.001; y += 1.0){
                    BreathingCube * c = new BreathingCube(g_world->m_shader, glm::vec3(x,y,0), glm::vec4(0.7f, 0.17f, 0.17f, 1.0f), cubesAreBreathing);
                    c->setRotation(cubeRotation, worldRelativeRotation);
                    g_world->addEntity(c);
                }
            }
        }
        else if(highPreformanceCube){
            for(float x = -3.0; x <= 3.0001; x+= 0.25){
                for (float y = -3.0; y <= 3.001; y += 0.25){
                    for (float z = -3.0; z <= 3.001; z += 0.25){
                        BreathingCube * c = new BreathingCube(g_world->m_shader, glm::vec3(x,y,z), glm::vec4(0.7f, 0.17f, 0.17f, 1.0f), cubesAreBreathing);
                        c->setRotation(cubeRotation, worldRelativeRotation);
                        g_world->addEntity(c);
                    }
                }
        ...
        else{ //3d 7*7*7 cube grid
            for(float x = -3.0; x <= 3.0001; x+= 1.0){
                for (float y = -3.0; y <= 3.001; y += 1.0){
                    for (float z = -3.0; z <= 3.001; z += 1.0){
                        BreathingCube * c = new BreathingCube(g_world->m_shader, glm::vec3(x,y,z), glm::vec4(0.7f, 0.17f, 0.17f, 1.0f), cubesAreBreathing);
                        c->setRotation(cubeRotation, worldRelativeRotation);
                        g_world->addEntity(c);
                    }
...
void setupWorld(GLFWwindow * window) {
    ...
    if(cubesAreScared){
        ...
        g_world->addEntity(new Light(g_world->m_shader, kLightPos, lightColor, moveLight));
    }
