...
public:
    BreathingCube(Shader * shader, glm::vec3 initial_pos, glm::vec4 initial_color, bool breathing = true) {
        m_type = ET_CUBE;
    ...
    void render() const {
        ...
        if (m_shader) { 
            ...
            float angle = 1 * m_rotation_scalar;
            glm::mat4 rotateCubeModelMat4 = glm::mat4();
            if(rotateWorld && !(m_rotation[0] < 0.01 && m_rotation[1] < 0.01 && m_rotation[2] < 0.01) ){
                rotateCubeModelMat4 = glm::rotate(glm::mat4(), angle, m_rotation);
            }
            glm::mat4 translateCubeModelMat4 = glm::translate(rotateCubeModelMat4 , glm::vec3(m_pos));
            glm::mat4 translatedScaledCubeModelMat4 = glm::scale(translateCubeModelMat4, glm::vec3(m_current_breath_amt));
            
            if(!rotateWorld && !(m_rotation[0] < 0.01 && m_rotation[1] < 0.01 && m_rotation[2] < 0.01) ){
                translatedScaledCubeModelMat4 = glm::rotate(translatedScaledCubeModelMat4, angle, m_rotation);
            }
    ...
    void update(double time_since_last_update) {
        ...
        if(isBreathing){
            m_current_breath_amt = 0.56 + 0.36 * sin(m_total_time + m_random_delta) + 0.001;
        }
        else{
            m_current_breath_amt = 0.9;
        }

        //change the color (red when big, blue when small)
        m_color[0] = 0.8 * m_current_breath_amt;
        m_color[1] = m_initial_color[1];
        m_color[2] = 1.0 - m_color[0];
